...
def convert_csv_to_json():
    ...
    to_process = products.copy()
    success_products = []
    failed_products = []
    max_total_retries = 3  # Total number of attempts including the initial processing
    current_retry = 0

    while current_retry < max_total_retries and to_process:
        logging.info(f"Starting processing attempt {current_retry + 1} with {len(to_process)} products")
        current_failed = []
        current_success = []

        for product_data in to_process:
            product_name = product_data['Product Name']
            description, short_description = get_ollama_description(product_name)
            default_description, default_short_description = generate_default_description(product_name)
            
            # Determine if the description is default (i.e., processing failed)
            if description == default_description and short_description == default_short_description:
                current_failed.append(product_data)
                logging.error(f"Failed to process product: {product_name}")
            else:
                product = create_product_dict(product_data, description, short_description)
                success_products.append(product)
                current_success.append(product_name)
                logging.info(f"Successfully processed product: {product_name}")
        
        # Log current attempt results
        logging.info(f"Attempt {current_retry + 1} completed: {len(current_success)} succeeded, {len(current_failed)} failed")
        if current_success:
            logging.debug(f"Successfully processed products in this attempt: {current_success}")
        if current_failed:
            logging.debug(f"Failed products in this attempt: {[p['Product Name'] for p in current_failed]}")
        
        # Prepare for next retry if there are failed products
        to_process = current_failed
        current_retry += 1
        if to_process and current_retry < max_total_retries:
            logging.info(f"Retrying {len(to_process)} failed products after delay...")
            time.sleep(2)  # Optional: delay before retrying
    
    # After all retries, check if there are still failed products
    if to_process:
        failed_products.extend(to_process)
        logging.error(f"Final failed products after {max_total_retries} attempts: {[p['Product Name'] for p in to_process]}")
    else:
        logging.info("All products processed successfully.")
    
    # Write successful products to JSON file
    try:
        with open('products.json', 'w', encoding='utf-8') as jsonfile:
            json.dump(success_products, jsonfile, indent=2, ensure_ascii=False)
        logging.info(f"Successfully wrote {len(success_products)} products to products.json")
    except Exception as e:
        logging.error(f"Failed to write JSON file: {str(e)}")
    
    # Optionally, handle failed products (e.g., write to a separate file)
    if failed_products:
        try:
            with open('failed_products.json', 'w', encoding='utf-8') as failfile:
                json.dump([p['Product Name'] for p in failed_products], failfile, indent=2, ensure_ascii=False)
            logging.info(f"Written {len(failed_products)} failed products to failed_products.json")
        except Exception as e:
            logging.error(f"Failed to write failed products JSON file: {str(e)}")
